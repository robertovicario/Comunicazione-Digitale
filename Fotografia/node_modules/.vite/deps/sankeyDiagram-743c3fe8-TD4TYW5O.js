...
// node_modules/mermaid/dist/sankeyDiagram-743c3fe8.js
function Tt(t) {
...
var Ot = (t) => (t = In.sanitizeText(t, Xn()), B[t] || (B[t] = new It(t), q.push(B[t])), B[t]);
var $t = () => q;
var zt = () => X;
var jt = () => ({
...
function Gt() {
  ...
  function E({ nodes: e, links: f }) {
    ...
    const l = new Map(e.map((h2, r) => [k(h2, r, e), h2]));
  ...
  function P({ nodes: e }) {
    const f = ot(e, (r) => r.depth) + 1, l = (i - t - u) / (f - 1), h2 = new Array(f);
    for (const r of e) {
      const _ = Math.max(0, Math.min(f - 1, Math.floor(s.call(null, r, f))));
      r.layer = _, r.x0 = t + _ * l, r.x1 = r.x0 + u, h2[_] ? h2[_].push(r) : h2[_] = [r];
    }
    if (o)
      for (const r of h2)
        r.sort(o);
  ...
  function I(e, f, l, h2) {
    for (; l < e.length; ++l) {
      const r = e[l], _ = (f - r.y0) * h2;
      _ > 1e-6 && (r.y0 += _, r.y1 += _), f = r.y1 + p;
    }
  }
  function g(e, f, l, h2) {
    for (; l >= 0; --l) {
      const r = e[l], _ = (r.y1 - f) * h2;
...
//# sourceMappingURL=sankeyDiagram-743c3fe8-TD4TYW5O.js.map
