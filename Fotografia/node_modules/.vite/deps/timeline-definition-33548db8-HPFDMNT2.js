...
// node_modules/mermaid/dist/timeline-definition-33548db8.js
var K = function() {
...
var it = () => A_;
var rt = function() {
  Q.length = 0, G.length = 0, R = "", V.length = 0, T_();
};
var at = function(n) {
  R = n, Q.push(n);
};
...
var Rt = function(n, t, e, a) {
  var W, b;
  const s = Xn(), c = s.leftMargin ?? 50;
  A.debug("timeline", a.db);
  ...
  A.debug("task", m), H.initGraphics(u);
  ...
  f.forEach(function(S) {
    ...
    A.debug("sectionHeight before draw", I), i = Math.max(i, I + 20);
  });
  let N = 0, A2 = 0;
  A.debug("tasks.length", m.length);
  for (const [S, _] of m.entries()) {
    ...
    A.debug("taskHeight before draw", P), r = Math.max(r, P + 20), N = Math.max(N, _.events.length);
    let C = 0;
  ...
  A.debug("maxSectionHeight before draw", i), A.debug("maxTaskHeight before draw", r), f && f.length > 0 ? f.forEach((S) => {
    ...
    A.debug("sectionNode", I);
    const P = u.append("g"), C = H.drawNode(P, I, w, s);
    A.debug("sectionNode output", C), P.attr("transform", `translate(${p}, ${d})`), l += i + 50, _.length > 0 && nt(
      u,
      _,
      w,
      p,
  ...
  A.debug("bounds", F), v && u.append("text").text(v).attr("x", F.width / 2 - c).attr("font-size", "4ex").attr("font-weight", "bold").attr("y", 20), h = x ? i + r + 150 : r + 100, u.append("g").attr("class", "lineWrapper").append("line").attr("x1", c).attr("y1", h).attr("x2", F.width + 3 * c).attr("y2", h).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)"), __(
    void 0,
    u,
...
var nt = function(n, t, e, a, s, c, o, y, g, u, m) {
  ...
  for (const f of t) {
    ...
    if (A.debug("taskHeight after draw", d), r.attr("transform", `translate(${a}, ${s})`), c = Math.max(c, d), f.events) {
...
var Wt = (n) => {
  ...
  for (let e = 0; e < n.THEME_COLOR_LIMIT; e++)
    n["lineColor" + e] = n["lineColor" + e] || n["cScaleInv" + e], ci(n["lineColor" + e]) ? n["lineColor" + e] = B(n["lineColor" + e], 20) : n["lineColor" + e] = L(n["lineColor" + e], 20);
  for (let e = 0; e < n.THEME_COLOR_LIMIT; e++) {
    const a = "" + (17 - 3 * e);
    t += `
...
//# sourceMappingURL=timeline-definition-33548db8-HPFDMNT2.js.map
